$(function() {
	...
	var Vy0 = 900 * radio, //初始y轴速度
		Vx0 = 0, //初始x轴速度
		ay = 1000 * radio, //初始y轴加速度
		ax = 0; //初始x轴加速度
	...
	for(var i = 0; i < 10; i++) {
		...
		stepsStyle();
		if(i == 0) {
			stairsL[i][1] = parseFloat(stepsHeight) + random(100,110); //第一个台阶bottom
		} else {
			stairsL[i][1] = parseFloat(stepsHeight) + random(100,110) + stairsL[i - 1][1] //第i+1个bottom值
		};
	...
	var wingCollideFn = function() {
		if(wingCollideB && wingE) {
			...
			if(Yt + 40 >= wingBottm && Yt <= wingBottm + 50) {
				...
				//此处为碰撞的接触距离,可以根据实际效果调整
				console.log(Xt + 10 <= wingLeft && Xt - 10 >= wingLeft)
				if(Xt + 10 <= wingLeft && Xt - 10 >= wingLeft) {
					console.log('碰撞到翅膀');
	...
	var rocketCollideFn = function() {
		if(rocketCollideB && rocketE) {
			...
			if(Yt + 40 >= rocketBottm && Yt <= rocketBottm + 50) {
				//此处为碰撞的接触距离,可以根据实际效果调整
				if(Xt + 10 <= rocketLeft + 50 && Xt - 10 >= rocketLeft -50) {
					//console.log('碰撞到翅膀');
	...
	var refreshFn = function() { //更新台阶
		if(stairsL[0][1] < -10) {
			stairsL.shift();
			stepsStyle();
			stairsL.push([Math.random(), stairsL[stairsL.length - 1][1] + parseFloat(stepsHeight) + random(100,110), stairsL[stairsL.length - 1][2] + 1]);
	...
	var refresh2Fn = function() { //向下更新台阶
		...
		if(stairsL[stairsLlen - 1][1] > winH + 20) {
			stairsL.pop();
			stairsL.unshift([Math.random(), stairsL[0][1] - parseFloat(stepsHeight) - random(100,110), stairsL[0][2] - 1]);
