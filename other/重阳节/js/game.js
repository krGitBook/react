$(function() {
	...
	var Vy0 = 1100 * radio, //初始y轴速度
		Vx0 = 0, //初始x轴速度
		ay = 1200 * radio, //初始y轴加速度
		ax = 0; //初始x轴加速度
	...
	var stairsFour=true;//台阶的index值是否小于4
	...
	for(var i = 0; i < 10; i++) {
		...
		stepsStyle();
		if(i == 0) {
			stairsL[i][1] = parseFloat(stepsHeight) + random(115,120); //第一个台阶bottom
		} else {
			stairsL[i][1] = parseFloat(stepsHeight) + random(115,120) + stairsL[i - 1][1] //第i+1个bottom值
		};
		...
		if(Vyt < 0 && collideB) {
			for(var i = 0; i < stairsL.length; i++) {
				if(Yt <= stairsL[i][1] + 85*radio && Yt - Vyt * t0 >= stairsL[i][1] + 85*radio) {//这个常数就是决定人物是否站在石板中间
					...
					if(Xt >= stairsL[i][0] - 201*radio && Xt <= stairsL[i][0] + 201*radio) { //211参考值    //left值是从左下角算起   bottom值是从最底部算起
						...
						if(Yt > 100 && onStageNumber != onStageNumberOld) { //触发画布相对运动
	...
	var wingFn = function() {
		if(wingB) {
			//翅膀的位置
			wingBottm = Math.random() * winH * 0.4*2 + winH;
			var wingLeftNot = [];
	...
	var wingCollideFn = function() {
		if(wingCollideB && wingE) {
			...
			if(Yt + 40 >= wingBottm && Yt <= wingBottm +  50) {
				...
				//此处为碰撞的接触距离,
				if(Xt+ 206*radio >= wingLeft && Xt<= wingLeft + 206*radio) {
					console.log('碰撞到翅膀');

					//碰到翅膀，改变跳跃图片
					$('.personAni').hide();
					$wing.css('background','none');
	...
	var wingUpFn = function() { //翅膀飞升
		...
		if(wingUpB) {
			...
			wingBottm -= falldownDis;
			$wing.css('bottom', '-='+falldownDis);

			knapsackBottm -= falldownDis;
	...
	var knapsackFn = function() {
		if(knapsackB) {
			//背包生成的位置
			knapsackBottm = Math.random() * winH * 0.3*2 + winH;
			var knapsackLeftNot = [];
			for(var i = 0; i < stairsL.length - 1; i++) {
	...
	var knapsackCollideFn = function() {
		if(knapsackCollideB && knapsackE) {
			...
			if(Yt + 40 >= knapsackBottm && Yt <= knapsackBottm + 50) {
				//此处为碰撞的接触距离,可以根据实际效果调整
				if(Xt + 152*radio >= knapsackLeft && Xt <= knapsackLeft + 152*radio) {
					//console.log('碰撞到背包');
					
					//碰到背包，改变跳跃图片
					$('.personAni').hide();
					$knapsack.css('background','none');
	...
	var rocketB = false;
	var rocketE = false; //有无火箭
	var rocketFn = function() {
		console.log()
		if(rocketB) {
			//火箭的位置
			rocketBottm = Math.random() * winH * 0.1*2 + winH;
			var rocketLeftNot = [];
			for(var i = 0; i < stairsL.length - 1; i++) {
	...
	var rocketCollideFn = function() {
		if(rocketCollideB && rocketE) {
			...
			if(Yt + 40 >= rocketBottm && Yt <= rocketBottm + 50) {
				//此处为碰撞的接触距离,可以根据实际效果调整
				if(Xt + 109*radio>= rocketLeft&& Xt<= rocketLeft + 109*radio) {
					//console.log('碰撞到火箭');

					//碰到火箭，改变跳跃图片
					$('.personAni').hide();
					$rocket.css('background','none');
	...
	var refreshFn = function() { //更新台阶
		if(stairsL[0][1] < -10) {
			stairsL.shift();
			stepsStyle();
			stairsL.push([Math.random(), stairsL[stairsL.length - 1][1] + parseFloat(stepsHeight) + random(115,120), stairsL[stairsL.length - 1][2] + 1]);
	...
	var refresh2Fn = function() { //向下更新台阶
		...
		if(stairsL[stairsLlen - 1][1] > winH + 20) {
			stairsL.pop();
			stairsL.unshift([Math.random(), stairsL[0][1] - parseFloat(stepsHeight) - random(115,120), stairsL[0][2] - 1]);
